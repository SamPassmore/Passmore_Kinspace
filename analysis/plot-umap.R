...
clusters$label_ = clusters[,paste0(kt, "_hamming")]
clusters = clusters[!is.na(clusters$label_),]

embeddings = read.csv(
  paste0("results/hdbscan/", kt, "_hamming.csv")
)
...
pp = ggplot(data = d2, aes(x = X, y = Y, label = label_)) + 
...
save(pp, file = paste0('results/umap/', kt,'_ggplot.rdata'))
